defmodule PhoenixClientSsl.Plug.ExtractCommonNameTest do
  ...
  describe "call/2" do
    test "skipps with already configure common name" do
      conn = %Conn{
        private: %{client_certificate: @otp_cert, client_certificate_common_name: :foo}
      }

      assert %Conn{private: %{client_certificate_common_name: :foo}} =
               ExtractCommonName.call(conn, %{})
    end

    test "extracts common name" do
      conn = %Conn{private: %{client_certificate: @otp_cert}}

      assert %Conn{private: %{client_certificate_common_name: "foo.bar.baz"}} =
               ExtractCommonName.call(conn, %{})
    end
